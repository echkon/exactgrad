...
for theta in angles:
    params[0] = theta
    grad = gradient_hst(hst_circuit, hst_cost, params)[0]
    hst_x.append(grad)
...
for s in angles:
    uvd = [[ghz_ansatz, 2, [], [s, np.pi]], [ghz_qc, 2, [], []]]
    ...
    grads_x.append(dev_cost_x[0])
    grads_2x.append(dev_cost_2x[0])
    grads_4x.append(dev_cost_4x[0])
    exact_x.append(exact_func(s, np.pi)[0])
