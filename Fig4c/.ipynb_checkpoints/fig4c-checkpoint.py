...
def generate_and_save_data():
    """
    Performs gradient calculations using various methods, computes the Mean Squared Error (MSE),
    ...
    # Initialize arrays with higher precision
    x_2point = np.zeros(len(step_sizes), dtype=np.float64)
    x_4point = np.zeros(len(step_sizes), dtype=np.float64)
    x_ht = np.zeros(len(step_sizes), dtype=np.float64)
    x_hst = np.zeros(len(step_sizes), dtype=np.float64)
    
    # Keeping original names for clarity, though not all are plotted in the example
    ...
    for i, value in enumerate(step_sizes):
        ...
        mse_2point_vals_x = np.zeros(step_number, dtype=np.float64)
        mse_4point_vals_x = np.zeros(step_number, dtype=np.float64)
        mse_ht_vals_x = np.zeros(step_number, dtype=np.float64)
        mse_hst_vals_x = np.zeros(step_number, dtype=np.float64)
        ...
        for j in range(step_number):
            ...
            mse_2point_vals_x[j] = np.square(np.abs(exact[0] - grad_2point[0]))
            mse_4point_vals_x[j] = np.square(np.abs(exact[0] - grad_4point[0]))
            mse_ht_vals_x[j] = np.square(np.abs(exact[0] - grad_ht[0]))
            mse_hst_vals_x[j] = np.square(np.abs(exact[0] - grad_hst[0]))
        ...
        # Mean Square Error
        x_2point[i] = np.mean(mse_2point_vals_x)
        x_4point[i] = np.mean(mse_4point_vals_x)
        x_ht[i] = np.mean(mse_ht_vals_x)
        x_hst[i] = np.mean(mse_hst_vals_x)
    ...
    with open('mse_data.pkl', 'wb') as f:
        pickle.dump(data_to_save, f)
        
    print("\nData generation complete. Results saved to mse_data.pkl")
